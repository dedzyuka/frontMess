...
struct ChatView: View {
    ...
    @AppStorage("lastSendButtonMode") private var lastSendButtonModeRaw = SendButtonMode.audio.rawValue
        ...
        mainContent
            ...
            .actionSheet(isPresented: $showingActionSheet) {
                ActionSheet(
                    title: Text("Вложение"),
                    buttons: [
                        .default(Text("Фото / Видео")) { showingMediaPicker = true },
            ...
            .sheet(isPresented: $showForwardChatSelection) {
                ChatSelectionForForwardView { selectedChat in
                    forwardMessageToChat(forwardMessage, targetChat: selectedChat)
                    forwardMessage = nil
                }
            }
    ...
    private var attachmentAndReplySection: some View {
        VStack(spacing: 0) {
            ...
            if chat.isPrivate, let otherUser = viewModel.otherUser {
                        ...
                        VStack(alignment: .leading, spacing: 2) {
                            ...
                            if viewModel.isSomeoneTyping {
                                ...
                                HStack(spacing: 4) {
                                    ...
                                    Text(viewModel.isOtherUserOnline ? "в сети" : "не в сети")
                                        .font(.caption)
                                        .foregroundColor(.secondary)
                                }
                            }
                ...
                HStack(spacing: 8) {
                    ...
                    VStack(alignment: .leading, spacing: 2) {
                        ...
                        Text(viewModel.isSomeoneTyping ? "печатает..." : "чат")
                            .font(.caption)
                            .foregroundColor(.secondary)
                    }
                }
    ...
    private func replyPreviewBar(_ reply: Message) -> some View {
        HStack(spacing: 8) {
    ...
    private var inputAreaView: some View {
        HStack(spacing: 12) {
            ...
            if isUploading || isSending {
                ...
                HStack(spacing: 8) {
                    Button(action: toggleRecorderMode) {
                        HStack(spacing: 6) {
                    ...
                    Button(action: openCurrentRecorder) {
    ...
    private func forwardMessageToChat(_ message: Message?, targetChat: Chat) {
        ...
        Task {
            ...
            if let pending = viewModel.pendingForward, let pendingAttachId = pending.attachmentId {
                ...
                do {
                    ...
                    let currentFileName = documentURL.lastPathComponent
                    fileName = currentFileName

                    let attributes = try FileManager.default.attributesOfItem(atPath: documentURL.path)
                    fileSize = attributes[.size] as? Int

                    mimeType = AttachmentUploader.shared.guessMimeType(from: currentFileName)
                } catch {
                    await MainActor.run {
                        isSending = false
                        isUploading = false
